...
def create_batch_advanced_signals(results_list, app=None):
    ...
    # Register trade button callbacks if app is provided
    if app:
        logger.info(f"Registering trade button callbacks with app")
        create_detail_button_callbacks(app)
    else:
        logger.warning("No app provided, trade button callbacks will not be registered")
    ...
    return html.Div([
        ...
        # Separate modal for trading plans
        dbc.Modal(
            [
                dbc.ModalHeader("Trading Plan"),
                dbc.ModalBody("Select a symbol to view trading plan"),
                dbc.ModalFooter(
                    dbc.Button("Close", id="close-advanced-trade-modal", className="ms-auto")
                ),
            ],
            id="advanced-trade-modal",
            size="xl",
            is_open=False,
        ),
            ...
            dbc.Table([
                ...
                html.Tbody([
                    html.Tr([
                        ...
                        # Actions
                        html.Td([
                            # Use dbc.Button with a pattern-matching ID format that doesn't conflict with main dashboard
                            dbc.Button(
                                "Trade", 
                                id={"type": "advanced-trade-btn", "index": data['symbol']},  # Pattern-matching ID
                                color="success",
                                size="sm",
...
def create_detail_button_callbacks(app):
    """Create individual callbacks for Trade buttons using simple string IDs."""
    global _batch_results
    
    if not _batch_results or not app:
        logger.warning("Cannot create trade button callbacks: missing results or app")
        return
        
    # Register callback for closing the trading plan modal
    @app.callback(
        Output("advanced-trade-modal", "is_open"),
        Input("close-advanced-trade-modal", "n_clicks"),
        prevent_initial_call=True
    )
    def close_trade_modal(n_clicks):
        """Close the trading plan modal when the close button is clicked"""
        if not n_clicks:
            raise PreventUpdate
            
        logger.info("Closing trading plan modal via close button")    
        return False
    
    logger.info(f"Setting up trade button callbacks for {len(_batch_results)} symbols")
    ...
    # Register one callback function for handling all trade buttons
    # This is more efficient than registering a callback per button
    @app.callback(
        Output("advanced-trade-modal", "is_open"),
        Output("advanced-trade-modal", "children"),
        Input({"type": "advanced-trade-btn", "index": dash.ALL}, "n_clicks"),
        prevent_initial_call=True
    )
    def show_batch_symbol_details(*n_clicks_list):
        """Handle clicks on any trade button to show trading plan"""
        # Figure out which button was clicked using callback_context
        ctx = dash.callback_context
        if not ctx.triggered:
            logger.warning("Trade button callback triggered but no button was clicked")
            raise PreventUpdate
        ...
        try:
            # Extract the button ID (pattern-matching format)
            button_id = json.loads(trigger.split('.')[0])
            # Extract symbol from pattern-matching format: {"type": "advanced-trade-btn", "index": SYMBOL}
            symbol = button_id.get('index')
            ...
            modal_content = [
                dbc.ModalHeader(f"Trading Plan for {symbol} @ {symbol_data.price:.2f}"),
                dbc.ModalBody(
                    details_view
                ),
                dbc.ModalFooter(
                    dbc.Button(
                        "Close", id="close-advanced-trade-modal", className="ms-auto", n_clicks=0
                    )
                ),
            ]
            ...
            logger.exception(f"Error showing trading plan for batch symbol: {e}")
            # Create an error modal instead
            error_modal_content = [
                dbc.ModalHeader("Error Displaying Trading Plan"),
                dbc.ModalBody([
